...
if [[ -n "$SEARCH_NAME" && -n "$RG" ]]; then
  # Reset clears the high-water-mark so the next run picks up
  # blobs uploaded "in time" with the indexer's first run (which
  # otherwise stick on the wrong side of the watermark and never
  # get indexed). Best-effort.
...
echo "  ./scripts/inspect_kb_contents.sh  (from terraform/2-deploy-aisoc/)"
